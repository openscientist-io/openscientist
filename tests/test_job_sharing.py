...
async def test_revoke_job_share(
    ...
    # Verify test_user2 can access while share exists
    await set_current_user(db_session, test_user2.id)
    stmt = select(Job).where(Job.id == job.id)
    result = await db_session.execute(stmt)
    assert result.scalar_one_or_none() is not None

    # Revoke share as owner (who has permission to delete)
    await set_current_user(db_session, test_user.id)
    await db_session.delete(share)
    await db_session.commit()

    # Verify test_user2 can no longer access after share is revoked
    await set_current_user(db_session, test_user2.id)
    stmt = select(Job).where(Job.id == job.id)
    result = await db_session.execute(stmt)
...
async def test_list_shared_with_me_jobs(
    ...
    share1 = JobShare(job_id=job1.id, shared_with_user_id=test_user2.id, permission_level="view")
    share2 = JobShare(job_id=job2.id, shared_with_user_id=test_user2.id, permission_level="edit")
