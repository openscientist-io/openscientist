...
def test_harness_env_names_mantle_without_proxy() -> None:
    # Inheriting the OpenAI-family default would send an unproxied omp run to
    # api.openai.com instead of Bedrock.
    with patch(
        "openscientist.providers.bedrock_openai.get_settings",
        return_value=_settings(region="us-west-2"),
    ):
        ...
        assert p.harness_env(proxy=None) == {
            ...
            "OPENAI_API_KEY": "br-key",
        }
        assert p.harness_env(proxy="http://openscientist:8081") == {
            "OPENAI_BASE_URL": "http://openscientist:8081"
        }


def test_proxy_env_overrides_replace_the_real_key() -> None:
    with patch("openscientist.providers.bedrock_openai.get_settings", return_value=_settings()):
        env = BedrockOpenAIProvider().proxy_env_overrides(
            proxy_base_url="http://openscientist:8081", placeholder="ph"
        )
    assert env == {
        "BEDROCK_API_KEY": "ph",
        "OPENAI_API_KEY": "ph",
