...
When ``settings.airgap.enabled`` is True, the factory selects
:class:`AirgapCodexAgent` for ``CodexCompatible`` providers and
:class:`AirgapClaudeCodeAgent` for ``ClaudeCompatible`` ones (unless the
managed-LLM egress opt-in, RFC §7.5 Pattern A, is set — see
``TestAirgapEnabled.test_claude_provider_allowed_with_managed_llm_egress``).
"""
...
class TestAirgapEnabled:
    ...
    def test_claude_returns_airgap_subclass(self, tmp_path: Path) -> None:
        from openscientist.agent.claude_code_agent import ClaudeCodeAgent
        ...
        with (
            ...
            # StubClaudeProvider's id 'stub' isn't in the egress registry;
            # the egress check itself is tested in test_egress_registry.py.
            # Here we patch it to assert the factory CALLS it (same wiring
            # discipline as the Codex path) and keep this test focused on
            # dispatch.
            patch(
                "openscientist.airgap.egress_registry.validate_provider_for_airgap",
                return_value={("10.0.0.5", 8443)},
        ...
        mock_validate.assert_called_once()
        called_args = mock_validate.call_args
        assert called_args.args[0] == "stub"  # provider id
        # Must be the airgap subclass, not the base.
        assert isinstance(agent, AirgapClaudeCodeAgent)
        # Sanity: still a ClaudeCodeAgent for downstream isinstance checks.
        assert isinstance(agent, ClaudeCodeAgent)
        assert agent.provider is provider

    def test_claude_refused_when_egress_validation_fails(self, tmp_path: Path) -> None:
        # The complement to the above — when validate_provider_for_airgap
        # raises, the factory must NOT instantiate AirgapClaudeCodeAgent.
        from openscientist.airgap.egress_registry import AirGapPolicyError
        ...
        with (
            ...
            patch(
                "openscientist.airgap.egress_registry.validate_provider_for_airgap",
                side_effect=AirGapPolicyError("Provider 'stub' would reach external endpoint"),
            ),
        ):
            with pytest.raises(AirGapPolicyError, match="would reach"):
                get_agent(AgentConfig(job_dir=tmp_path))
