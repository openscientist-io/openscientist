...
**Status:** Draft v4 — for review (no implementation merged yet, but PR-1 scaffold is underway). Revised after four adversarial Codex reviews.
**Date:** 2026-06-05
...
+ per-backend application-layer enforcement (gated SDK built-ins for Claude; fork-level web-search
gating for Codex, §8.2) + credential minimization (active-provider-only allowlist) + output
...
| **Codex CLI LLM call** | direct to OpenAI / Azure OpenAI | rejected at startup unless internal endpoint (§7); no egress route exists outside the allowlist (§6, §8) |
| **Codex CLI built-in web search** | enabled by default | disabled at the Codex fork binary level for every air-gap-supported provider (§8.2, §10) |
...
9. **Codex backend reconfigured via `AirgapCodexAgent` subclass.** `agent/factory.py` selects
   ...
   - never copy host `auth.json` — the operator provisions it via a read-only mount.
   `agent/codex_agent.py` itself gets at most ~20 lines of refactor to make those helpers
   overridable; **its public contract is unchanged**. No network/web-search toggle exists on the
   SDK to set — see §8.2 for why, and what enforces those instead.
...
| Network controls | gated SDK built-in tools | kernel/firewall isolation (§6); web search gated at the Codex fork level, not an SDK flag |
...
contract the Codex CLI subprocess speaks. Only Ollama is actually validated against this contract
(a live Tier-4 test against real hardware, plus Luca's own compatibility fixes in #195). Other OpenAI-compatible
serving stacks — vLLM, llama.cpp's HTTP server, TGI, SGLang — advertise the same `/v1/responses`
surface on paper, but per Luca's review, each implementation is "unstandardized" enough that
compatibility isn't a given. The operator chooses one based on their hardware budget;
gpt-oss-120b runs on a single H100 (or quantized on consumer hardware), gpt-oss-20b on much less.
The same pattern works with any open-weight model that ships a compliant Responses-API surface.

**This isn't speculative.** A live Tier-4 validation already ran this end-to-end against a
...
There is no SDK-level `network_access_enabled` / `web_search_enabled` toggle to set — the package
this RFC originally targeted (an early, unofficial `openai_codex_sdk`) had a `ThreadOptions`
dataclass with those fields, but the official `openai-codex` package (what actually shipped, via
PR #195) doesn't have one; `thread_start()` takes keyword arguments directly, with no network
knob at all. That's not a gap: network isolation is enforced at the kernel/firewall layer (§6)
regardless of what the SDK does, and web-search capability is gated inside the Codex CLI fork
binary itself (not advertised to any of the CodexCompatible providers air-gap mode supports) —
both stronger guarantees than an SDK flag that could silently no-op. Air-gap mode instead adds
the overrides that remain load-bearing:
...
class AirgapCodexAgent(CodexAgent):
    ...
    Overrides the helper methods that build the MCP env, relocate CODEX_HOME,
    skip host-auth copying, and filter the Codex CLI subprocess's own env,
    leaving the public contract identical.
    """

    def _mcp_env(self) -> dict[str, str]:
        # Allowlist-construct instead of os.environ.copy()
        return filtered_agent_env(dict(os.environ), self._provider.id)

    def _codex_home(self) -> Path:
        # Per-job tmpfs, outside the job_dir export tree
        return Path("/run/openscientist-codex-home") / self._job_dir().name

    def _ensure_auth(self) -> None:
        # No-op: the operator provisions auth.json via a read-only host mount;
        # the agent process never copies anything from the host filesystem.
        return None

    def _make_codex(self) -> AsyncCodex:
        # Launch the Codex CLI subprocess with the same allowlist-filtered env
        # used for _mcp_env, not raw os.environ.
        ...
```

`agent/codex_agent.py` exposes `_mcp_env`, `_codex_home`, `_ensure_auth`, and `_make_codex` as
overridable helpers, but its public method signatures don't change.

`agent/factory.py` adds one conditional (~3 lines): if `settings.airgap.enabled and isinstance(provider, CodexCompatible)`, instantiate `AirgapCodexAgent` instead of `CodexAgent`.

### 8.3 Verification

The per-job attestation (§14) records the image digest of the running agent/executor containers
and the Codex CLI binary's SHA256, so a verifier can confirm the deployed image — including its
compiled-in web-search gate — matches what was audited. Network isolation itself is verified at
the container/firewall layer (§6, §14), not by inspecting SDK call arguments, since no such
arguments exist. The attestation also scans the generated `config.toml` for forbidden secret
patterns.
...
Per §8.2 via the subclass: kernel/firewall network isolation, fork-level web-search gating, MCP env
...
- From **inside the executor container**, attempted connection to *anything* — fails (no network
  namespace egress)
- **Codex CLI probes:** `config.toml` does not contain forbidden secrets; `auth.json` is not in
  the job dir; the running Codex CLI binary digest matches the attested fork build (§8.1, §8.3),
  confirming its compiled-in web-search gate is in effect
...
| `tests/airgap/test_codex_agent_airgap.py` | asserts `_codex_home`/`_mcp_env`/`_ensure_auth` overrides, env allowlist |
