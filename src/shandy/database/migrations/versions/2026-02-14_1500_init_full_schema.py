...
def upgrade() -> None:
    ...
    op.create_index(op.f("ix_mcp_servers_is_enabled"), "mcp_servers", ["is_enabled"], unique=False)
    ...
    op.execute(
        """
        CREATE POLICY mcp_servers_select_all ON mcp_servers FOR SELECT
            USING (
                current_setting('app.current_user_id', TRUE) IS NOT NULL
                AND is_enabled = true
            )
    ...
    op.execute(
        """
        CREATE POLICY mcp_servers_admin ON mcp_servers FOR ALL
            USING (current_user_is_admin())
    """
    )
