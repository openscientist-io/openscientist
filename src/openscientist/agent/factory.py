...
def get_agent(config: AgentConfig) -> AbstractAgent[Provider]:
    ...
    operator's allowlist before the agent is instantiated:
    `AirgapCodexAgent` for `CodexCompatible` providers,
    `AirgapClaudeCodeAgent` for `ClaudeCompatible` ones (RFC §10.3 SDK
    built-in gating).
    """
    ...
    if isinstance(provider, ClaudeCompatible):
        ...
        if settings.airgap.allow_managed_llm_egress:
            # Pattern A escape hatch (RFC §7.5): managed-LLM egress (e.g.
            # Bedrock under AWS BAA for HIPAA use cases) opts a
            # ClaudeCompatible provider into the *regular* ClaudeCodeAgent
            # instead of AirgapClaudeCodeAgent below — kernel-level
            # allowlist is the only enforcement boundary, SDK built-ins
            # are ungated. Documented gap; VPC endpoint (Pattern B) is
            # the migration target. Takes precedence over the normal
            # airgap dispatch because it's a narrower, explicit opt-in
            # layered on top of airgap mode.
        ...
        # Normal airgap path: AirgapClaudeCodeAgent, same egress-validation
        # gate as the Codex path below.
        ...
        logger.info("Using AirgapClaudeCodeAgent (air-gap mode) with provider %s", provider.id)
        return AirgapClaudeCodeAgent(config, provider)
