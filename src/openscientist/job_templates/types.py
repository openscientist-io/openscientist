...
Guided templates are a submit-time UX layer: they collect structured inputs and
compose a research question plus methodology guidance. Both ride the existing
job fields (``research_question`` and ``description``), so no runtime or schema
changes are needed — the agent receives the guidance through the normal prompt
path.
"""
...
class JobTemplate:
    ...
    def build_guidance(self, inputs: Mapping[str, Any]) -> str:
        """Render methodology guidance for the discovery agent.

        This is stored in the job's ``description`` field, which the
        orchestrator already injects into every iteration prompt.
        """
        input_lines = []
        for field in self.fields:
...
class TemplateResolution:
    """Validated job template submission ready for job creation."""
