...
def login_page() -> None:
    ...
    with (
        ...
        # Animated OpenScientist logo
        ui.html(OPENSCIENTIST_THINKING_SVG, sanitize=False).classes("w-16 h-16").style(
            "width:64px;height:64px;min-width:64px;min-height:64px;"
        )

        # Title and subtitle
        ui.label("OpenScientist").classes("text-3xl font-bold text-cyan-800")
        ...
        # OAuth buttons container - only show configured providers
        with ui.column().classes("gap-3 w-72"):
            # Google OAuth button
            if google_enabled:
                with (
                    ui.button(on_click=lambda: ui.navigate.to("/auth/google/login"))
            ...
            if not oauth_enabled and not mock_enabled:
                with (
                    ui.card().classes("w-full bg-yellow-50 border-l-4 border-yellow-500"),
